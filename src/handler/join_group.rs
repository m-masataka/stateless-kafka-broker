...
pub async fn handle_join_group_request(
    ...
    log::info!(
        "Handling JoinGroupRequest API VERSION {}",
    ...
    let mut consumer_group = meta_store
        .update_consumer_group(group_id.as_str(), move |mut cg| async move {
    ...
    let response = if let Some(store_group) = consumer_group.as_mut() {
        ...
        if store_group.is_rebalancing {
            log::warn!(
                "Consumer group {} is currently rebalancing, rejecting join request",
                *group_id
            );
            ...
            let cg = new_consumer_group(
                request,
                member_id_str,
                leader_id,
                &group_id,
                generation_id,
                store_group.topics.clone(),
            );
            meta_store.save_consumer_group(&cg).await?;

            convert_consumer_group_to_join_response(&cg, requester_id, 0)
        } else {
...
pub fn convert_consumer_group_to_join_response(
    ...
    let members: Vec<JoinGroupResponseMember> = cg
        ...
        .map(|m| {
            ...
            resp_member.metadata = m.metadata.clone().unwrap_or_default();
...
fn new_consumer_group(
    request: &JoinGroupRequest,
    member_id: String,
    leader_id: String,
    group_id: &StrBytes,
    generation_id: i32,
    topics: Option<Vec<ConsumerGroupTopic>>,
) -> ConsumerGroup {
    let generation_id = generation_id;
    let protocol_type: StrBytes = request.protocol_type.clone();

    // 🔍 protocol_name
    let protocol_name: StrBytes = request
        .protocols
        .first()
        .map(|p| p.name.clone())
