    ...
    W: AsyncWrite + Unpin + Send,
{
    ...
    response.topics = request.topics
        ...
        .map(|topic| {
            ...
            match meta_store.get_topic_info(Some(topic.name.as_ref()), None) {
                Ok(Some(store_topic)) => {
                    log::info!("Topic already exists: {}", topic_result.topic_id);
                    ...
                    meta_store.save_topic_partition_info(&topic_metadata);
                },
                Ok(None) => {
