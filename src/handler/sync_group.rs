...
pub async fn handle_sync_group_request(
    ...
    log::info!(
        "Handling SyncGroupRequest API VERSION {}",
    ...
    let consumer_group = meta_store
        .update_consumer_group(&group_id, move |mut cg| async move {
    ...
    let response = if let Some(_) = consumer_group {
        let protocol_type: StrBytes = request
            .protocol_type
            .clone()
