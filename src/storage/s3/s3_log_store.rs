...
impl UnsendLogStore for S3LogStore {
    async fn write_records(
        ...
        if let Some(data) = records {
            ...
            self.s3_client
                .put_object(&self.bucket, &object_key, &output, None)
                .await
                .map_err(|e| {
            ...
            log::debug!(
                "Successfully wrote batch to S3 for topic: {}, partition: {}",
                topic_id,
                partition
            );
...
impl S3LogStore {
    fn log_key(&self, topic_id: &str, partition: i32, offset: i64) -> String {
        match &self.prefix {
            Some(prefix) => format!(
                "{}/{}/{}/{}{}.log",
                prefix, topic_id, partition, self.log_file_name_prefix, offset
            ),
            None => format!(
                "{}/{}/{}{}.log",
